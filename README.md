<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=12,20,24&height=190&section=header&text=Kayumkhan%20Sayal&fontSize=46&fontColor=ffffff&animation=fadeIn&fontAlignY=36&desc=Full%20Stack%20Product%20Engineer%20%7C%20Problem%20Solver&descAlignY=56&descSize=17" width="100%" alt="header" />

<a href="https://github.com/itskayumkhan">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=23&duration=3200&pause=900&color=00D9FF&center=true&vCenter=true&width=620&lines=Full+Stack+Product+Engineer+from+India;Next.js+%7C+Node.js+%7C+PostgreSQL;Leading+a+team+of+10%2B+developers;Ideas+to+production-ready+architecture" alt="Typing SVG" />
</a>

<br/><br/>

<a href="https://kayumkhan-sayal.netlify.app/"><img src="https://img.shields.io/badge/Portfolio-00D9FF?style=for-the-badge&logo=netlify&logoColor=white" alt="Portfolio" /></a>
<a href="https://linkedin.com/in/kayumkhan_sayal"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" /></a>
<a href="mailto:kayumkhansayal2004@gmail.com"><img src="https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" /></a>
<a href="https://twitter.com/kayumkhan_sayal"><img src="https://img.shields.io/badge/Twitter-1D9BF0?style=for-the-badge&logo=x&logoColor=white" alt="Twitter" /></a>

<br/>

<img src="https://komarev.com/ghpvc/?username=itskayumkhan&label=Profile%20views&color=00d9ff&style=flat-square" alt="profile views" />
<img src="https://img.shields.io/github/followers/itskayumkhan?label=Followers&style=flat-square&color=00d9ff" alt="followers" />

</div>

<br/>

## About Me

I am a **Full Stack Product Engineer** based in India, building fast, scalable and secure applications with **Next.js, Node.js and PostgreSQL**. Currently pursuing my MCA while leading a team of **10+ developers**.

I am not a developer who just clears tickets. I take raw ideas and turn them into production ready architecture: database schemas, API pipelines, responsive UI, deployment. Complete ownership of the product, not a slice of it.

My working style is **player-coach**. Deeply hands on with the codebase, writing clean code, while mentoring the team to ship faster without breaking production. I do my best work in high growth startup environments, building things real users depend on.

```javascript
const kayum = {
  role: "Full Stack Product Engineer",
  location: "India",
  currently: ["MCA", "Leading a team of 10+ devs"],
  focus: "Turning raw ideas into production-ready systems",
  frontend: ["Next.js (App Router)", "React", "TypeScript", "Tailwind CSS"],
  backend: ["Node.js", "Express", "PostgreSQL", "MongoDB"],
  architecture: ["System Design", "API Design", "AI Data Pipelines"],
  askMeAbout: ["Next.js", "system design", "shipping without breaking prod"]
};
```

<br/>

## Tech Arsenal

<table>
<tr>
<td valign="top" width="33%">

**Frontend**

<img src="https://skillicons.dev/icons?i=nextjs,react,ts,tailwind&perline=4" alt="frontend" />

Next.js App Router, React, TypeScript, Tailwind CSS

</td>
<td valign="top" width="33%">

**Backend and Database**

<img src="https://skillicons.dev/icons?i=nodejs,express,postgres,mongodb&perline=4" alt="backend" />

Node.js, Express, PostgreSQL, MongoDB

</td>
<td valign="top" width="33%">

**Architecture and Tools**

<img src="https://skillicons.dev/icons?i=git,vercel,postman,linux&perline=4" alt="tools" />

System Design, API Design, AI Data Pipelines

</td>
</tr>
</table>

<div align="center">

<img src="https://skillicons.dev/icons?i=js,ts,python,cpp,html,css,bash,redux,prisma,supabase,figma,github,vscode,netlify&perline=14" alt="all tech" />

</div>

<br/>

## Featured Work

### [client-context](https://github.com/itsKayumkhan/claude-client-context)

<a href="https://github.com/itsKayumkhan/claude-client-context"><img src="https://img.shields.io/github/stars/itsKayumkhan/claude-client-context?style=flat-square&color=00d9ff&labelColor=0d1117" alt="stars" /></a>
<img src="https://img.shields.io/github/license/itsKayumkhan/claude-client-context?style=flat-square&color=00d9ff&labelColor=0d1117" alt="license" />
<img src="https://img.shields.io/github/last-commit/itsKayumkhan/claude-client-context?style=flat-square&color=00d9ff&labelColor=0d1117" alt="last commit" />

A Claude Code skill that turns raw client chat into a versioned project knowledge base. Every requirement gets a status, every run diffs against the last one, and conflicts get flagged instead of silently resolved. Open source, MIT licensed.

### [Samsung Distributor Store](https://github.com/itsKayumkhan/patrick-samsung-store)

<img src="https://img.shields.io/badge/Next.js-000000?style=flat-square&logo=nextdotjs&logoColor=white" alt="Next.js" />
<img src="https://img.shields.io/badge/Prisma-2D3748?style=flat-square&logo=prisma&logoColor=white" alt="Prisma" />
<img src="https://img.shields.io/badge/Supabase-3FCF8E?style=flat-square&logo=supabase&logoColor=0d1117" alt="Supabase" />
<img src="https://img.shields.io/badge/Stripe-635BFF?style=flat-square&logo=stripe&logoColor=white" alt="Stripe" />

A distributor e-commerce store built end to end: catalog and inventory schema, checkout with Stripe, and an admin side for order handling.

<br/>

## Connect

<div align="center">

<a href="https://kayumkhan-sayal.netlify.app/"><img src="https://img.shields.io/badge/Portfolio-00D9FF?style=for-the-badge&logo=netlify&logoColor=0d1117" alt="Portfolio" /></a>
<a href="https://linkedin.com/in/kayumkhan_sayal"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" /></a>
<a href="mailto:kayumkhansayal2004@gmail.com"><img src="https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" /></a>
<a href="https://github.com/itskayumkhan"><img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" /></a>

<a href="https://leetcode.com/kayumkhan_sayal"><img src="https://img.shields.io/badge/LeetCode-FFA116?style=for-the-badge&logo=leetcode&logoColor=0d1117" alt="LeetCode" /></a>
<a href="https://auth.geeksforgeeks.org/user/kayumkhan_sayal"><img src="https://img.shields.io/badge/GeeksforGeeks-2F8D46?style=for-the-badge&logo=geeksforgeeks&logoColor=white" alt="GeeksforGeeks" /></a>
<a href="https://twitter.com/kayumkhan_sayal"><img src="https://img.shields.io/badge/Twitter-000000?style=for-the-badge&logo=x&logoColor=white" alt="Twitter" /></a>
<a href="https://instagram.com/kayumkhan_sayal"><img src="https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white" alt="Instagram" /></a>
<a href="https://fb.com/kayumkhan_sayal"><img src="https://img.shields.io/badge/Facebook-1877F2?style=for-the-badge&logo=facebook&logoColor=white" alt="Facebook" /></a>

<br/>

**kayumkhansayal2004@gmail.com**

Open to high growth startup roles and interesting product problems.

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=12,20,24&height=110&section=footer" width="100%" alt="footer" />

</div>
